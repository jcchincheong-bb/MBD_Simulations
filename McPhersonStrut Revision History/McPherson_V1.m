...
%% Constraint Equation Formulation (Manual-ish)
Phi = [];                                           % Empty constraint equation array
PhiA = rev_pin_phi(r{1},r{2},s{1,2},s{1,3});
PhiB = tran_phi(r{2},r{3},u{3},u{4},s{2,3},s{3,4});  % Translational Joint u3 is primary axis
PhiO = rev_pin_phi([0;0],r{3},s{3,1},s{3,4});
PhiQ = rev_pin_phi([0;0],r{1},s{4,1},s{4,2});
Phi = [PhiA;PhiB;PhiO;PhiQ;0];

%% Evaluating Constraint Equations
Phi_sol = subs(Phi,q,q_0);
T = table(vpa(Phi_sol),'VariableNames',{'Constraint'});
disp(T)